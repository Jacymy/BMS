...
public class UserUpkeep extends PublicJFrame {
  ...
  private JPanel createDataPanel() {
    ...
    jc_isAdmin = new JComboBox<>(admin);//创建“用户权限”组合框，并提供选择项
  ...
  private JPanel createButtonPanel() {
    ...
    JButton jb_insert = new JButton("添加");//创建“添加”按钮
    //为“添加”按钮添加单击事件监听器
    jb_insert.addActionListener(_ -> {
    ...
    JButton jb_update = new JButton("修改");//创建“修改”按钮
    //为“修改”按钮添加单击事件监听器
    jb_update.addActionListener(_ -> {
    ...
    JButton jb_delete = new JButton("删除");//创建“删除”按钮
    // 为删除按钮添加单击事件监听器
    jb_delete.addActionListener(_ -> {
    ...
    JButton jb_cancel = new JButton("取消");//创建“取消”按钮
    // 为取消按钮添加单击事件监听器
    jb_cancel.addActionListener(_ -> {
    ...
    JButton jb_close = new JButton("关闭");//创建“关闭”按钮
    // 为关闭按钮添加单击事件监听器
    jb_close.addActionListener(_ -> {
      dispose();// 关闭“用户维护界面”
    });
    ...
    JButton jb_empty = new JButton("清空所有用户");//创建“清空所有数据”按钮
    // 为清空按钮添加单击事件监听器
    jb_empty.addActionListener(_ -> {
  ...
  private JPanel createTablePanel() {
    ...
    for (User value : list) {// 将user表中的用户添加到表格模型中
      model.addRow(new Object[]{value.getId(), value.getName(), value.getPassword(),
        value.getIs_admin()});
    }
  ...
  public void refresh() {
    ...
    // 重新向表格模型中添加数据
    for (User user : list) {
      model.addRow(new Object[]{user.getId(), user.getName(), user.getPassword(),
        user.getIs_admin()});
    }
  ...
  public void jb_insert_actionPerformed() {
    ...
    user.setPassword(new String(pf_pass.getPassword()).trim());// 设置用户对象的密码
    int is_admin = 2;// 获取“用户权限”
    if (Objects.equals(Objects.requireNonNull(jc_isAdmin.getSelectedItem()).toString(), "管理员")) {
      is_admin = 1;
    } else {
    ...
    if (tf_name.getText().trim().isEmpty()
      || new String(pf_pass.getPassword()).trim().isEmpty()) {
      //若空，则给出提示，并返回
  ...
  public void jb_update_actionPerformed() {
    ...
    user.setPassword(new String(pf_pass.getPassword()).trim());
    int is_admin = 2;// 获取“用户权限”
    if (Objects.requireNonNull(jc_isAdmin.getSelectedItem()).toString().equals("管理员")) {
      is_admin = 1;
    } else {
      if (Objects.equals(jc_isAdmin.getSelectedItem().toString(), "操作员")) {
        is_admin = 0;
      }
    }
