...
public class MainFrame extends PublicJFrame {
  ...
  private JMenuBar createMenuBar() {
    ...
    //添加单击事件监听器
    mi_user_upkeep.addActionListener(_ -> {
    ...
    mi_reader_upkeep.addActionListener(_ -> {
    ...
    mi_book_upkeep.addActionListener(_ -> {
    ...
    // 声明读者查询菜单项
    JMenuItem mi_reader_query = new JMenuItem("读者查询"); // 创建"读者查询"菜单项
    mi_reader_query.addActionListener(_ -> {
      reader_query_actionPerformed();// 调用"读者查询"响应方法
    });
    menu_query.add(mi_reader_query);

    // 声明图书查询菜单项
    JMenuItem mi_book_query = new JMenuItem("图书查询"); // 创建"图书查询"菜单项
    mi_book_query.addActionListener(_ -> {
      book_query_actionPerformed();// 调用"图书查询"响应方法
    });
    menu_query.add(mi_book_query);

    mi_book_statistics = new JMenuItem("图书统计"); // 创建" 图书统计"菜单项
    mi_book_statistics.addActionListener(_ -> {
    ...
    mi_update_pass.addActionListener(_ -> {
      update_pass_actionPerformed();// 调用"修改密码"响应方法
    });
    menu_management.add(mi_update_pass);

    // 声明退出系统菜单项
    JMenuItem mi_exit = new JMenuItem("退出系统"); // 创建"退出系统"菜单项
    mi_exit.addActionListener(_ -> {
  ...
  private JToolBar createToolBar() {
    ...
    // 创建"读者维护"图标
    ImageIcon icon_reader_upkeep = new ImageIcon("E:\\Java 综合项目设计\\BMS\\src\\images\\reader_upkeep.png");
    bt_reader_upkeep.setIcon(icon_reader_upkeep);// 为"读者维护"按钮设置图标
    bt_reader_upkeep.setToolTipText("读者维护");// 为"读者维护"按钮设置提示
    // 将"读者维护"按钮注册单击事件监听器
    bt_reader_upkeep.addActionListener(_ -> {
      reader_upkeep_actionPerformed();// 调用"读者维护’响应方法
    });
    //第3步：将按钮添加到工具栏上
    ...
    ImageIcon icon_book_upkeep = new ImageIcon("E:\\Java 综合项目设计\\BMS\\src\\images\\book_upkeep.png");
    bt_book_upkeep.setIcon(icon_book_upkeep);
    toolBar.add(bt_book_upkeep);
    bt_book_upkeep.addActionListener(_ -> {
      book_upkeep_actionPerformed();// 调用“图书维护”响应方法
    });

    bt_borrow = new JButton("借书");
    ImageIcon icon_borrow = new ImageIcon("E:\\Java 综合项目设计\\BMS\\src\\images\\borrow.png");
    bt_borrow.setIcon(icon_borrow);
    toolBar.add(bt_borrow);
    bt_borrow.addActionListener(_ -> {
      borrow_actionPerformed();// 调用“借书”响应方法
    });

    bt_back = new JButton("还书");
    ImageIcon icon_back = new ImageIcon("E:\\Java 综合项目设计\\BMS\\src\\images\\back.png");
    bt_back.setIcon(icon_back);
    toolBar.add(bt_back);
    bt_back.addActionListener(_ -> {
      back_actionPerformed();// 调用“还书”响应方法
    });

    // 声明读者查询按钮
    JButton bt_reader_query = new JButton("读者查询");
    ImageIcon icon_reader_query = new ImageIcon("E:\\Java 综合项目设计\\BMS\\src\\images\\reader_query.png");
    bt_reader_query.setIcon(icon_reader_query);
    toolBar.add(bt_reader_query);
    bt_reader_query.addActionListener(_ -> {
      reader_query_actionPerformed();// 调用“读者查询”响应方法
    });

    // 声明图书查询按钮
    JButton bt_book_query = new JButton("图书查询");
    ImageIcon icon_book_query = new ImageIcon("E:\\Java 综合项目设计\\BMS\\src\\images\\book_query.png");
    bt_book_query.setIcon(icon_book_query);
    toolBar.add(bt_book_query);
    bt_book_query.addActionListener(_ -> {
      book_query_actionPerformed();// 调用“图书查询”响应方法
    });

    bt_book_statistics = new JButton("图书统计");
    ImageIcon icon_book_statistics = new ImageIcon("E:\\Java 综合项目设计\\BMS\\src\\images\\book_statistics.png");
    bt_book_statistics.setIcon(icon_book_statistics);
    toolBar.add(bt_book_statistics);
    bt_book_statistics.addActionListener(_ -> {
      book_statistics_actionPerformed();// 调用“图书统计”响应方法
    });

    // 声明退出系统按钮
    JButton bt_exit = new JButton("退出系统");
    ImageIcon icon_exit = new ImageIcon("E:\\Java 综合项目设计\\BMS\\src\\images\\exit.png");
    bt_exit.setIcon(icon_exit);
    toolBar.add(bt_exit);
    bt_exit.addActionListener(_ -> {
      exit_actionPerformed();// 调用“退出系统"响应方法
    });
    //第4步：返回工具栏
    return toolBar;
  ...
  private void exit_actionPerformed() {
    BaseDao.closeConnection();// 关闭与数据库的连接
    System.exit(0);// 退出系统
  }
