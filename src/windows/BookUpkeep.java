...
public class BookUpkeep extends PublicJFrame {
  ...
  private JPanel createButtonPanel() {
    ...
    JButton jb_empty = new JButton("清空所有图书");
    //为清空按钮添加单击事件监听器
  ...
  private JPanel createTablePanel() {
    ...
    for (Book value : list) {
      model.addRow(new Object[]{value.getId(), value.getName(),
        value.getType(), value.getAuthor(),
        value.getTranslator(), value.getPublisher(),
        value.getPublish_time(), value.getStock(),
        value.getPrice()});
    }
  ...
  public void refresh() {
    ...
    for (Book value : list) {// 重新项表格模型中添加数据
      model.addRow(new Object[]{value.getId(), value.getName(),
        value.getType(), value.getAuthor(),
        value.getTranslator(), value.getPublisher(),
        value.getPublish_time(), value.getStock(),
  ...
  public void table_mouseClicked() {
    // 获取选中图书的信息
    Book book_old = list.get(table.getSelectedRow());
    // 在数据面板中设置对应的图书数据
  ...
  public void jb_insert_actionPerformed() {
    ...
    book.setType(Objects.requireNonNull(cb_type.getSelectedItem()).toString());
    ...
    book.setStock(Integer.parseInt(tf_stock.getText().trim()));
