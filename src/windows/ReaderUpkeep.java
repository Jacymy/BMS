...
public class ReaderUpkeep extends PublicJFrame {
  ...
  private JPanel createButtonPanel() {
    ...
    JButton jb_close = new JButton("关闭");
    // 为关闭按钮添加单击事件监听器
    ...
    JButton jb_empty = new JButton("清空所有读者");
    // 为清空按钮添加单击事件监听器
  ...
  public void refresh() {
    ...
    for (Reader value : list) {// 重新项表格模型中添加数据
      model.addRow(new Object[]{value.getId(),
        value.getName(), value.getType(),
        value.getSex(), value.getMax_num(),
        value.getDays_num()});
  ...
  public void table_mouseClicked() {
    // 获取选中读者的信息
    Reader reader_old = list.get(table.getSelectedRow());
    // 在数据面板中设置对应的数据
  ...
  public void jb_insert_actionPerformed() {
    ...
    reader.setType(Objects.requireNonNull(cb_type.getSelectedItem()).toString());
    reader.setSex(Objects.requireNonNull(cb_sex.getSelectedItem()).toString());
    reader.setMax_num(Integer.parseInt(tf_max_num.getText().trim()));
    reader.setDays_num(Integer.parseInt(tf_days_num.getText().trim()));
