...
public class Login extends PublicJFrame {
   ...
   * 成员变量
   */
  private final JTextField tf_user;// 定义用户名文本框
  private final JPasswordField pf_pass;// 定义密码框
  ...
  Login() {
    ...
    // 定义登录按钮
    JButton bt_login = new JButton("登录");// 创建登录按钮
    bt_login.setBounds(new Rectangle(100, 160, 80, 25));
    // 第3步：为按钮添加单击事件监听器
    // 为“登录”按钮添加单击事件监听器
    bt_login.addActionListener(_ -> {
      // 调用“登录”按钮事件响应方法
    ...
    // 定义关闭按钮
    JButton bt_close = new JButton("关闭");// 创建关闭按钮
    bt_close.setBounds(new Rectangle(200, 160, 80, 25));
    // 为“关闭”按钮添加单击事件监听器
    bt_close.addActionListener(_ -> {
...
//			System.out.println("进入系统主界面");
      ...
      main.setPurView(user.getIs_admin());//此方法在系统主界面中定义
